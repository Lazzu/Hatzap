...
void main( void )
{
	...
	float lumaTL = dot(luma, texture2D(textureSampler, tcoord.xy + (vec2(-1.0, -1.0) * texCoordOffset)).xyz);
	float lumaTR = dot(luma, texture2D(textureSampler, tcoord.xy + (vec2(1.0, -1.0) * texCoordOffset)).xyz);
	...
	vec3 result1 = (1.0/2.0) * (
		texture2D(textureSampler, tcoord.xy + (dir * vec2(1.0/3.0 - 0.5))).xyz +
	...
	if(lumaResult2 < lumaMin || lumaResult2 > lumaMax)
		RGBA = vec4(result1, 1.0);
	else
		RGBA = vec4(result2, 1.0);
